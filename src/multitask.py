    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for (images, labels) in target_domain[1]:
                ...
                acc += (predicted == labels).sum().item()
                total += labels.size(0)
            target_acc = acc / total
        print(
            f'Epoch {epoch + 1}/{epochs} classification loss: {loss} | source acc: {source_acc}, target acc: {target_acc} | time elapsed: {time.time() - start}')
        history.append((loss, source_acc, target_acc))
    if save_model:
        np.save(f"results/multitask_{name}", history)
...
if __name__ == '__main__':
    ...
    transform2 = transforms.Compose([transforms.Resize((64, 64)), transforms.Grayscale(num_output_channels=3), transforms.ToTensor()])
    ...
    multitask_runner(source_domain, target_domain, "mnist_usps", save_model=False)
